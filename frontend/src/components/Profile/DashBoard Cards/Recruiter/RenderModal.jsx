  ...
  async function handleSubmit(){
    ...
    try{
        ...
        if(!data ||data.status==="Failure" ||data.status==="Error"){
          alert("Failed to update the selected applicant job status")
          return
        }

        console.log(data);

        dispatch(setNotification("User Profile Status Updated"))

        setTimeout(() => {
          setToggleModal(false); // Close modal after 500ms
        }, 1000);
...
return (
  <>
    {isNotification && <Notification message={notificationMsg}></Notification>}
